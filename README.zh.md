...
Yorulog 属于 Yoru 系列轻量级 STM32 工具库。

这些库采用相近的设计原则：尽量保持单头文件、低依赖、低资源占用，并优先面向 STM32 HAL 工程。它们之间没有强制依赖关系，可以单独使用，也可以按项目需要组合使用。
...
这些库可以独立接入，也可以组合成更完整的调试、配置和片上存储基础设施：
